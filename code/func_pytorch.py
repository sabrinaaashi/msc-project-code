...
def set_parameters(goal,bifurcation,parentDir):
    ...
    #adjacency matrix
    A= torch.from_numpy(np.load(parentDir+"/src_and_example/in_data/SC_m12av_voxNorm_th20.npy")).float()
    #print(A)
    parameters['A'] = A
...
def set_random_initial_conds_for_nodes(bifurcation,noise_strength,tsteps,d,dt,parameters,realizations,noise):
    ...
    A=parameters['A']
    N=parameters['N']
    nocontrol = torch.zeros((tsteps + 4000, N))

    # Set random initials on a circle with set amplitudes
    if bifurcation == 'low':
        ini_y = torch.rand(N) / 2 + 0.1
        ini_z = torch.rand(N) / 5 + 0.5
    elif bifurcation == 'high':
        ini_y = torch.rand(N)
...
def initialize_noise(noise_strength,dt,realizations,tsteps,N):
    ...
    """
    sqrt_dt=dt**(1/2)
    noise =  (noise_strength/sqrt_dt)*torch.randn(realizations, tsteps, N)
    return noise
...
def read_node_inis_from_file(bifurcation,switchfromto,fileDir,tsteps,d,dt,parameters):
    ...
    ic_state_low=torch.from_numpy(content['state_low']).float()
    ic_state_high=torch.from_numpy(content['state_high']).float()
    #print(ic_state_low,ic_state_high)
...
def covariance_matrix(x , tsteps , N):
    ...
    xcc = (x-means)
    covariance_mat= torch.matmul(xcc.transpose(0, 1), xcc)
...
def f_sync_noisy(control,tsteps , d , dt , N , I_p , I_e , I_s , alpha , beta, gamma, delta, epsilon, tau, mu ,sigma, A,  target_output ,node_ic,realizations,noise):
    ...
    f1=0
    for realization in torch.arange(realizations):
...
def nabla_x_f_sync(state, tsteps,N,  target_output,  I_p):
    ...
    means = (1/tsteps) * torch.sum(state,dim=0)
    out=torch.zeros((tsteps,N))
    xm= state-means
    cc=cross_correlation_matrix(state , tsteps , N)
    stds =  (((state-means)**2).sum(axis=0))**(1/2) 
    cov_mat=covariance_matrix(state,tsteps,N)
    for ts in torch.arange(tsteps):
...
def nabla_u_f_sparsity(control,adjoint,tsteps,dt,N,I_e,I_s):
    ...
    out=torch.zeros((tsteps,N))
    controlsum= torch.sum(control**2, dim=0)**(1.0 / 2.0)#vecnorm(control,2,0)

    for i in torch.arange(N):
        for j in torch.arange(tsteps):
...
def ode_AS(phi,ts, tsteps , d, dt , N , alpha , beta, gamma, delta, epsilon, tau, mu ,sigma, A, state,nabla_x):
    ...
    """ 
    tss=-ts-1
    out=torch.zeros(phi.shape)
    Coupling = torch.matmul(A,phi[0])
    out[0]= Rder(state[tss], alpha , beta, gamma) *phi[0] + phi[1]/torch.tensor(tau) + nabla_x[tss]  + sigma * Coupling 
    out[1]= -phi[0] - (torch.tensor(epsilon)/torch.tensor(tau))*phi[1]
    return out 
